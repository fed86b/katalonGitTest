...
public class CompareImages {
	...
	public static boolean imgCompare(def imgOriginal,def imgToCompareWithOriginal,String imgOutputDifferences="", int size=70, int threshhold=0.5) {
	...
	public static compareWebElementPic(TestObject element,EnumRole role,def name="download.jpg",def inIframe=true,def pic_center=true) {
		...
		if(pic_center){
			...
			if(role==EnumRole.CONTENT_MANAGER&&inIframe){
				x+=301
				y+=140
			}
		}
		...
		CompareImages.imgCompare(desired, actual,WebHelper.getLocation("difference"))
	...
	public static detailedComparison(def stage="",def isLogin=false){
		...
		imgCompare(desired_loc, actual_loc,differences_loc)
	}
}
